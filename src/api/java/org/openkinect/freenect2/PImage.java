...
//     * @see PApplet#createImage(int, int, int)
    ...
    public void updatePixels(int x, int y, int w, int h) {  // ignore
        ...
        if (!modified) {
            mx1 = max(0, x);
            mx2 = min(pixelWidth, x2);
            my1 = max(0, y);
            my2 = min(pixelHeight, y2);
            modified = true;

        } else {
            if (x < mx1) mx1 = max(0, x);
            if (x > mx2) mx2 = min(pixelWidth, x);
            if (y < my1) my1 = max(0, y);
            if (y > my2) my2 = min(pixelHeight, y);

            if (x2 < mx1) mx1 = max(0, x2);
            if (x2 > mx2) mx2 = min(pixelWidth, x2);
            if (y2 < my1) my1 = max(0, y2);
            if (y2 > my2) my2 = min(pixelHeight, y2);
        }
    }
...
//        target.parent = parent;  // parent may be null so can't use createImage()
    ...
    public void mask(int maskArray[]) {  // ignore
        ...
        if (maskArray.length != pixels.length) {
            throw new IllegalArgumentException("mask() can only be used with an image that's the " +
                                               "same size.");
        }
        for (int i = 0; i < pixels.length; i++) {
            ...
            case GRAY:
                if (format == ALPHA) {
                    ...
                    for (int i = 0; i < pixels.length; i++) {
                        ...
                        // 0.59 * 256 = 151
                        // 0.11 * 256 =  28
                        int lum = (77 * (col >> 16 & 0xff) + 151 * (col >> 8 & 0xff) + 28 * (col
                                                                                             & 0xff)) >> 8;
    ...
    protected void dilate() {  // formerly dilate(false)
        ...
        while (index < maxIndex) {
            ...
            while (index < maxRowIndex) {
                ...
                int lumLeft =
                        77 * (colLeft >> 16 & 0xff) + 151 * (colLeft >> 8 & 0xff) + 28 * (colLeft
                                                                                          & 0xff);
                int lumRight =
                        77 * (colRight >> 16 & 0xff) + 151 * (colRight >> 8 & 0xff) + 28 *
                                                                                      (colRight & 0xff);
                int lumUp =
                        77 * (colUp >> 16 & 0xff) + 151 * (colUp >> 8 & 0xff) + 28 * (colUp & 0xff);
                int lumDown =
                        77 * (colDown >> 16 & 0xff) + 151 * (colDown >> 8 & 0xff) + 28 * (colDown
                                                                                          & 0xff);
...
int testFunction(int dst, int src) {
   ...
   */
    ...
    */
    ...
    /*
     * comment out the save method so far, Eric
     */
//
//    /**
